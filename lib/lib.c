...
void write_members(FILE *a, member *arr, int size) {
  ...
  for (int i = 0; i < size; ++i)
    if (db_members[i].account_to_delete == true)
      size--;
  fwrite(&size, sizeof(int), 1, a);

  for (int i = 0; i < size; ++i) {
    const member *m = &arr[i];
    if (m->account_to_delete)
      continue;
...
date date_wizard() {
  ...
  // tip
  printf(
      "Ensure any values of 0 are formatted like '0' not '00' or otherwise\n");
...
member *id_to_member_ptr(const int a) { return &db_members[a]; }
...
// returns member to the id provided
member id_to_member(const int a) { return db_members[a]; }
...
int member_add(const member a) {
  printf("Before add: index=%d, capacity=%d\n", db_members_index,
         db_members_capacity);
  if (db_members_index == db_members_capacity) {
    db_members_capacity =
        (db_members_capacity == 0) ? 1 : db_members_capacity * 2;
    printf("Reallocating to new capacity: %d\n", db_members_capacity);
  ...
  db_members_index++;
  printf("created new member at index %d, new index=%d, capacity=%d\n",
         db_members_index - 1, db_members_index, db_members_capacity);
  return db_members_index - 1;
}

int member_add_index(const member a, const int b) {
  if (b >= db_members_capacity) { // Check against capacity
    printf("Index %d exceeds capacity %d\n", b, db_members_capacity);
    return 0;
  }
  db_members[b] = a;
  return 1;
...
int loan_add(const loan a) {
  if (db_loans_index == db_loans_capacity) {
    db_loans_capacity = (db_loans_capacity == 0) ? 1 : db_loans_capacity * 2;
    loan *temp = realloc(db_loans, db_loans_capacity * sizeof(loan));
    if (!temp) {
  ...
  if (db_loans == NULL) {
    printf("Error db_loans is uninitialized\n");
    return -2;
  }
